...
//Helper
func convertStringToMatrix(_ string: String) -> [[Int]]? {
    ...
    for line in lines {
        ...
        var row = [Int]()
        for value in values {
            guard let int = Int(value.trimmingCharacters(in: .whitespaces)) else {
                return nil
            }
            row.append(int)
        }
...
//PART 1
func calcChecksum(_ string: String) -> Int {
    guard let rows = convertStringToMatrix(string) else {
        return 0
    }
    
    return rows.reduce(0) { (sum, next) in
        sum + rowChecksum(next)
    }
}

func rowChecksum(_ row: [Int]) -> Int {
    guard var min = row.first else {
        return 0
    }
...
//PART 2
func calcChecksum2(_ string: String) -> Int {
    guard let rows = convertStringToMatrix(string) else {
        return 0
    }
    
    return rows.reduce(0) { (sum, next) in
        sum + rowChecksum2(next)
    }
}

func rowChecksum2(_ row: [Int]) -> Int {
    for var i in 0..<row.count-1 {
        for var j in i+1..<row.count {
            let dividend = max(row[i], row[j])
            let divisor = min(row[i], row[j])
